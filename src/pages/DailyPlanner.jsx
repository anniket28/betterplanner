...
export default function DailyPlanner(props) {
  ...
  const [tasks, setTasks] = useLocalStorageWithReset("dailyTasks", "daily");
      ...
      <div className="space-y-3">
        ...
        {tasks.map(task => (
          <div key={task.id} className="flex items-center justify-between bg-white p-3 rounded-lg shadow">
            ...
            <button onClick={() => deleteTask(task.id)} className="text-red-500 hover:text-red-700"><FiTrash2 /></button>
          </div>
        ))}
      </div>
    </div>
